...
<head>
<meta charset="UTF-8">
<title><?php echo ($about_type["type_name"]); ?></title>
<meta content="<?php echo ($about_type["keyword"]); ?>" name="keywords" />
<meta content="<?php echo ($about_type["description"]); ?>" name="description" />
...
<?php echo (str_replace('\"','"',$param_array[0])); endforeach; endif; else: echo "" ;endif; ?>
		...
		<div class="pright">
			<div class="location">
				<span class="locat">
					您当前的位置：<?php echo ($about_type["type_name"]); ?>
				</span>
				<h2><?php echo ($about_type["type_name"]); ?></h2>
			</div>
			<div class="probox">

		<br><br>
        <div> <?php echo (str_replace('\"','"',$about["news_content"])); ?></div>
        <br/>
...
<?php if(is_array($lunboanli_list)): $i = 0; $__LIST__ = $lunboanli_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $param_array=split('[|]',$vo['param']); ?>   
            <li>
            <a  href="<?php echo U('index/anli_show',array('id'=>$vo['id'])); ?>" title="<?php echo (msubstr($vo["product_title"],0,28,"utf-8",true)); ?>" class="group-bak">
            <div class="pic-bak"><img src="/timthumb.php?src=<?php echo ($vo["product_pic_s"]); ?>&h=197&w=242&zc=2" height="197" width="242" ></div>
            <h5><?php echo (msubstr($vo["product_title"],0,28,"utf-8",true)); ?></h5></a>
