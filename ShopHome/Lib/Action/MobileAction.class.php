...
//		//中部推荐案例
		...
		$dongtailist = $product->where ( $condition )->limit ( 0,4 )->order ( ' show_level,id desc ' )->select ();
		$this->assign ( "dongtailist", $dongtailist );
		//行业资讯
		$condition ["type_id"] = 144;
		$zixunlist = $product->where ( $condition )->limit ( 0,4 )->order ( ' show_level,id desc ' )->select ();
...
//		$this_mytype_id=I ( "id" ) ?I ( "id" ): 143;
		...
		$ModualNtype = M ( "Product_type" );
		$mymodualNtype = $ModualNtype->find ( I ( "type_id" ) );
