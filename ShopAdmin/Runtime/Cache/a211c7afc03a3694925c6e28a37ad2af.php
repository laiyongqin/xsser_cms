      ...
      <div class="title">
        ...
        <div class="content">
          <ul id="tabExample1" class="nav nav-tabs">
                                    <li class="active"><a href="#home1" data-toggle="tab">为管理组中【<?php echo ($mymodual["name"]); ?>】这个组配置单页和新闻权限</a></li>
           </ul>
 ...
 <?php if(($vo["type_type"]) == "0"): ?><font style="color: blue;">单页栏目</font><?php endif; if(($vo["type_type"]) == "1"): ?><font style="color: red;">新闻栏目</font><?php endif; ?>
              ...
              <td class="ms">
                <div class="btn-group1">
                  <form class="form-horizontal row-fluid" action="<?php echo U("ConfigAdmin/quanxian_news_update");?>" method="post">
                  <input type="hidden" name="power_id" value="<?php echo ($mymodual["id"]); ?>" />
                  <input type="hidden" name="nid" value="<?php echo ($vo["id"]); ?>" />
                  <input type="checkbox" value="<?php $temp_this_add=getadmin_quanxian_caozuon($mymodual['id'],$vo['id'],'add'); echo $temp_this_add; ?>" name="add[]" <?php if ($temp_this_add==1){echo 'checked="CHECKED"';} ?> >增加</label>
                  <input type="checkbox" value="<?php $temp_this_del=getadmin_quanxian_caozuon($mymodual['id'],$vo['id'],'del'); echo $temp_this_del; ?>" name="del[]" <?php if ($temp_this_del==1){echo 'checked="CHECKED"';} ?> >删除</label>
                  <input type="checkbox" value="<?php $temp_this_up=getadmin_quanxian_caozuon($mymodual['id'],$vo['id'],'up'); echo $temp_this_up; ?>" name="up[]" <?php if ($temp_this_up==1){echo 'checked="CHECKED"';} ?> >修改</label>
                  <input type="checkbox" value="<?php $temp_this_select=getadmin_quanxian_caozuon($mymodual['id'],$vo['id'],'select'); echo $temp_this_select; ?>" name="select[]" <?php if ($temp_this_select==1){echo 'checked="CHECKED"';} ?> >查询</label>
