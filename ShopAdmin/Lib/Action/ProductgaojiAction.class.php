<?php
class ProductgaojiAction extends BaseAction {
	
	public $c_letf_number = 5;
	...
	public function index() {
		...
		import ( '@.ORG.Tool' );
		//所有板块
		$thislistsql = "SELECT pt.*  FROM  `xsser_product_type` as pt   LEFT JOIN   `xsser_modual` as mo   ON  mo.`id`=pt.`modual_id` WHERE  mo.`modual_type`=1 AND pt.`type_type` <100  order by `type_parent` asc,`type_type` asc,`type_rank` asc,`show_level` asc ";
		...
		$this->display ( 'productgaoji/index' ); //输出页面模板
	}
	...
	public function getlist() {
		...
		$c_letf_number = 5;
		$this->assign ( "c_letf_number", $c_letf_number );
		$this->display ( 'productgaoji/list' ); //输出页面模板
	}
	...
	public function update_info() {
		...
		$pModel = M ( ucfirst ( $mymodualNtype ['database_table'] ) ); //....
		$param_array = $pModel->find ( $mymodual ["param"] );
		$param_array_table_field = split ( '[|]', $mymodualNtype ['table_field'] ); //....
		$this->assign ( "param_array", $param_array );
		$this->assign ( "param_array_table_field", $param_array_table_field );
		...
		$c_letf_number = 5;
		$this->assign ( "c_letf_number", $c_letf_number );
		$this->display ( 'productgaoji/update' ); //输出页面模板
	}
	...
	public function del() {
		...
		if ($Modual->where ( 'id=%d', $_GET ['id'] )->delete ()) {
			//高级模型产品删除后，把连带的xsser_new_modual_XXXXX表中数据也去掉防止占据数据库空间
